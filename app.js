...
const JisraApi = require("jira-client");
...
const fs = require("fs");
const { Collection, Ticket } = require("./config/bd");
...
//     res.redirect('/pageLogin/login.html')
//   }
//   else {
//     next();
//   }
...
app.post("/createTicket", async (req, res) => {
  try {
    ...
    let users = await jiraClient.userSearch.findUsers({ query: authEmail });
    let newUser = users[0];

    if (!users || users.length === 0) {
      console.log("User not found, creating new user:", authEmail);
      newUser = await jiraClient.users.createUser({
    ...
    const issueData = {
      fields: {
        ...
        // assignee: { emailAddress: userEmail },
        customfield_10038: url, // Adjust this field key as needed
        customfield_10039: {
          value: priority,
        },
        customfield_10040: {
    ...
    // console.log(customfield_10038);

    const created = await jiraClient.issues.createIssue(issueData);
   
    const issue = await jiraClient.issues.getIssue({ issueIdOrKey: created.id });
    console.log(issue);
    ...
    res.status(201).json({ issueKey: issue.key });
  } catch (error) {
    console.error(
...
app.get("/api/search", async (req, res) => {
  ...
  try {
    ...
    if (!items.length) {
      return res
        .status(404)
        .json({ error: "No items found for the tag or name" });
    }
...
app.get("/tickets",authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (tickets) {
      const  syncTickets = await tickets.map(async x => {
        let issue  ;
        try {
          issue = (await jiraClient.issues.getIssue({ issueIdOrKey: x.issueKey }))
        } catch (error) {
          return null
        }
        x.url = issue.fields['customfield_10038'].value;
        x.priority = issue.fields['customfield_10039'].value;
        x.status = issue.fields['customfield_10040'].value;
        return x 
      })
      res.json( (await Promise.all(syncTickets)).filter(x => x));
    } else {
...
app.get("/items/all/:collectionid", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Ошибка при получении всех элементов из коллекции" });
  }
});
...
app.get("/collections/top", async (req, res) => {
  try {
    ...
    const topCollectionsWithItemCount = await Promise.all(
      topCollections.map(async (collection) => {
        const itemCount = await Item.count({
          where: { collectionid: collection.id },
        }); // Замените collectionId на collectionid
...
app.post("/users/login", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      redirectUrl: "/html/mainPage.html",
    });
  } catch (error) {
...
app.delete("/users/delete/:userId", async (req, res) => {
  ...
  try {
    ...
    const result = await sequelize.query("DELETE FROM users WHERE id = $1", {
      bind: [userId],
...
app.put(
  "/users/update_information/:userId",
  authMiddleware,
  isProfileOwner,
  async (req, res) => {
    const userId = parseInt(req.params.userId);
    ...
    try {
      ...
      if (!username && !email) {
        return res
          .status(400)
          .json({ message: "Не указаны поля для обновления" });
      }
      ...
      if (result.rowCount > 0) {
        res
          .status(200)
          .json({ message: "Информация о пользователе успешно обновлена" });
      } else {
        res.status(404).json({ message: "Пользователь не найден" });
      }
    } catch (error) {
      ...
      res
        .status(500)
        .json({ message: "Ошибка при обновлении информации о пользователе" });
    }
  }
);
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (userResult.length === 0) {
        return res
          .status(404)
          .json({ error: "User Not Found", message: "Пользователь не найден" });
      }
      ...
      if (oldPassword !== currentPassword) {
        return res
          .status(401)
          .json({ error: "Unauthorized", message: "Неверный старый пароль" });
      }
      ...
      await sequelize.query(updateQuery, {
        ...
        type: sequelize.QueryTypes.UPATE,
      });
